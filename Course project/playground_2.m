...
alpha = 2;
image_original = imread('cropped_1.jpg');
...
for i=1:6
    b_replacement(:, :, i) = imresize(b(:, 1:end-1, i), [size(m{1, 1},1) size(m{1, 1},2)], "bicubic");
end

b_replacement = {b_replacement};

imrec = idualtree2(imresize(image_lower_res, size(image_original), "bicubic"), b_replacement);
