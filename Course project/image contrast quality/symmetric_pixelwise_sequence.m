function W=symmetric_pixelwise_sequence(X,N,theta,units,period,window_shape)
%%
...
% toolbox     :  \map\maputils\deg2rad.m
% Last updated: 20OCT2015
...
for k=1:N
    if k<=N/2+1
        ...
        AD(:,:,k)=conv2(conv2(X,dex,'same'),dey,'same');
%         A(:,:,k)=AD(:,:,k).*AI(:,:,k);
        if k<=4
            A(:,:, 2.*k-1) = AD(:,:,k);
            A(:,:, 2.*k) = AI(:,:,k);
...
function result = CRTE(vorudi, alpha)
    ...
    input_sorted = sort(vorudi, 2);
    for i=1:8
        result = (1./(alpha-1)).*(1-alpha.*((1-i./8).^(alpha-1)))./8.*input_sorted(:, i);
    end
end
