function x = my_idddtree2(dt, resized_image)
    ...
    % Inverse 2-D Dual-Tree Complex Discrete Wavelet Transform  
        for j = 1:L
            for d = 1:3
                ...
                A = imresize(cfs{j}(:,:,d,1,1), 1,"bicubic");
                B = imresize(cfs{j}(:,:,d,2,1), 1,"bicubic");
                C = imresize(cfs{j}(:,:,d,2,2), 1, "bicubic");
                D = imresize(cfs{j}(:,:,d,1,2), 1, "bicubic");
                temp = zeros([size(A,1)*1, size(A,2)*1 1 2 2]);
                % These are in terms of filtering operations
                temp(:,:,d,1,1) = imresize((A+B)/sqrt(2), 1, "bicubic");
                temp(:,:,d,2,2) = imresize((A-B)/sqrt(2), 1, "bicubic");
                temp(:,:,d,1,2) = imresize((C-D)/sqrt(2), 1, 'bicubic');
                temp(:,:,d,2,1) = imresize((C+D)/sqrt(2), 1, "bicubic");
            end
            cfs{j} = temp;
        end
        ...
        for k = 1:2
            for n = 1:2
                ...
                for j = L:-1:1
                    ...
                    y = recFB(y,imresize(cfs{j}(:,:,:,n,k), 2),recF1,recF2);
                end
                x = x+imresize(y, 1);
            end
        end
...
% filter along rows
Lo = local_REC_FB(Lo,imresize(Hi(:,:,1), 1),Rf2,2);
Hi = local_REC_FB(imresize(Hi(:,:,2), 1),imresize(Hi(:,:,3), 1),Rf2,2);
