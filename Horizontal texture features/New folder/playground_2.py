...
def noise_remove(file_path, intensity_factor, std, blurring_kernel_dim, MF_dim , Mean_F_dim, K):
    ...
    rec_image4 = np.abs(
        np.fft.ifft2((((np.abs(kernel_blur_fft) ** 2) / (np.abs(kernel_blur_fft) ** 2 + K)) / kernel_blur_fft) *
    ...
    plt.title(f'Full Wiener filtered, dim={MF_dim}', fontname='Times New Roman', fontweight="bold")
