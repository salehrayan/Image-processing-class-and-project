...
def periodic_noise_remove(file1_path):
    ...
    notchpair2_y = 654 - width / 2
    notchpair3_x = 369 - height / 2
    notchpair3_y = 511 - width / 2
    notchpair4_x = 302 - height / 2
    notchpair4_y = 368 - width / 2
    ...
    D3 = np.hypot(X - notchpair3_x, Y - notchpair3_y)
    D_3 = np.hypot(X + notchpair3_x, Y + notchpair3_y)
    D4 = np.hypot(X - notchpair4_x, Y - notchpair4_y)
    D_4 = np.hypot(X + notchpair4_x, Y + notchpair4_y)

    NRfilter = (1 / (1 + (D0 / D1) ** (2 * n))) * (1 / (1 + (D0 / D_1) ** (2 * n))) * \
               (1 / (1 + (D0 / D2) ** (2 * n))) * (1 / (1 + (D0 / D_2) ** (2 * n))) * \
               (1 / (1 + (D0 / D3) ** (2 * n))) * (1 / (1 + (D0 / D_3) ** (2 * n))) * \
               (1 / (1 + (D0 / D4) ** (2 * n))) * (1 / (1 + (D0 / D_4) ** (2 * n)))
