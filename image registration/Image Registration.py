...
def image_registeration(file_path):
    shape = 400
    image = cv2.imread(file_path, cv2.IMREAD_GRAYSCALE)
    image  = cv2.resize(image, (shape, shape), interpolation= cv2.INTER_CUBIC)
    ...
    sheared_image = cv2.warpPerspective(image, shear_vertical_matrix, (image.shape[1],
                                        image.shape[0]+int(image.shape[0]*shear_vertical_factor)))
    ...
    difference = abs(image - restored_image_mean)
