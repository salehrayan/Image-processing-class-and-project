...
def inverse_fourier(file1_path, file2_path, intensity_factor):
    ...
    plt.subplot(3, 4, 7)
    plt.imshow(minmax(np.abs(np.fft.ifft2(np.abs(image1_fft)))) ** 1.5, cmap="gray", vmin=0, vmax=255)
    plt.title(f'Image 1 rec. from spectrum', fontname='Times New Roman', fontweight="bold")

    plt.subplot(3, 4, 8)
    plt.imshow(minmax(np.abs(np.fft.ifft2(np.abs(image2_fft)))) ** 1.5, cmap="gray", vmin=0, vmax=255)
...
if __name__ == "__main__":
    ...
    file2 = r'C:\Users\ASUS\Desktop\Image processing\Rotate and resize\standard_test_images\woman_darkhair.tif'
    inverse_fourier(file1, file2, intensity_factor=2.1)
