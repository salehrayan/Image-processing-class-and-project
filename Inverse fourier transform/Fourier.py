...
def inverse_fourier(file1_path, file2_path, intensity_factor, T=None):
    ...
    plt.subplot(3, 4, 9)
    plt.imshow(minmax(np.abs(np.real(np.fft.ifft2(image1_fft_phaseinfo)))), cmap="gray", vmin=0, vmax=255)
    plt.title(f'Image 1 rec. from phase', fontname='Times New Roman', fontweight="bold")

    plt.subplot(3, 4, 10)
    plt.imshow(minmax(np.abs(np.real(np.fft.ifft2(image2_fft_phaseinfo)))), cmap="gray", vmin=0, vmax=255)
    plt.title(f'Image 2 rec. from phase.', fontname='Times New Roman', fontweight="bold")

    plt.subplot(3, 4, 11)
    plt.imshow(minmax(np.abs(np.real(np.fft.ifft2(im1phase_im2spec)))), cmap="gray", vmin=0, vmax=255)
    plt.title(f'Rec. with Image 1 phase\nand Image 2 mag. spectrum', fontname='Times New Roman', fontweight="bold")

    plt.subplot(3, 4, 12)
    plt.imshow(minmax(np.abs(np.real(np.fft.ifft2(im2phase_im1spec)))), cmap="gray", vmin=0, vmax=255)
...
if __name__ == "__main__":
    ...
    inverse_fourier(file1, file2, intensity_factor=2, T=image_T)
