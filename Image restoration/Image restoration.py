...
from Histogram_matching import histogram_match_fn
...
def noise_remove(file_path, intensity_factor, std, blurring_kernel_dim, MF_dim , Mean_F_dim):
    ...
    rec_image_k1 = np.abs(
        np.fft.ifft2((((np.abs(kernel_blur_fft) ** 2) / (np.abs(kernel_blur_fft) ** 2 + 1)) / kernel_blur_fft) *
    ...
    rec_image_kpoint1 = np.abs(
        np.fft.ifft2((((np.abs(kernel_blur_fft) ** 2) / (np.abs(kernel_blur_fft) ** 2 + 0.1)) / kernel_blur_fft) *
                     image_blur_noise_fft))
    ...
    rec_image_kpoint5 = np.abs(
        np.fft.ifft2((((np.abs(kernel_blur_fft) ** 2) / (np.abs(kernel_blur_fft) ** 2 + 0.5)) / kernel_blur_fft) *
                     image_blur_noise_fft))
    ...
    rec_image_k10 = np.abs(
        np.fft.ifft2((((np.abs(kernel_blur_fft) ** 2) / (np.abs(kernel_blur_fft) ** 2 + 10)) / kernel_blur_fft) *
                     image_blur_noise_fft))
    ...
    for row in tqdm(range(MF_dim-2,image_blur_noise.shape[0]+(MF_dim-2))):
        for col in range(MF_dim-2, image_blur_noise.shape[1]+(MF_dim-2)):
            ...
            mu = np.mean(temp)
            sigma2 = np.var(temp)
            rec_image3[row, col] = mu + ((sigma2 - std**2)/sigma2)*(image_blur_noise_padded[row, col]-mu)
...
if __name__ == '__main__':
    ...
    noise_remove(file_path, intensity_factor=1, std=20, blurring_kernel_dim=4,
